...
for _ in range(shipments_amount):
    ...
    # realistic delivery time assuming average speed of 60 km/h
    base_delivery_time = round(distance / 60, 2)
    
    # random variation to delivery time
    ...
    if status == 'Cancelled':
        # for cancelled shipments possible admin fees
        admin_fee = round(random.uniform(50, 200), 2)
        
        cursor.execute("""
        INSERT INTO costs (shipment_id, fuel_cost, toll_cost, maintenance_cost, admin_fee)
        VALUES (?, ?, ?, ?, ?)
        """, (shipment_id, 0, 0, 0, admin_fee))
        
        revenue_amount = round(admin_fee * 1.5, 2)
        ...
        # assuming 5.5 PLN per liter
        fuel_cost = round((distance * fuel_consumption / 100) * 5.5, 2)
        ...
        INSERT INTO costs (shipment_id, fuel_cost, toll_cost, maintenance_cost, admin_fee)
        VALUES (?, ?, ?, ?, ?)
        """, (shipment_id, fuel_cost, toll_cost, maintenance_cost, 0))
        
        total_cost = fuel_cost + toll_cost + maintenance_cost
        base_profit_margin = random.uniform(1.2, 1.5)
